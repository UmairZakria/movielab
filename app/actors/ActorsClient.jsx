...
const ActorsClient = ({ trendingInitial, popularInitial }) => {
  const [trending, setTrending] = useState(trendingInitial || []);
  const [popular, setPopular] = useState(popularInitial || []);
  const [loading, setLoading] = useState(!trendingInitial || trendingInitial.length === 0);
  ...
  const fetchMorePopular = async () => {
    ...
    try {
      ...
      const res = await axios.get(
        `${BASE_URL}/person/popular?api_key=${API_KEY}&include_adult=false&page=${nextPage}`,
      );
  ...
  useEffect(() => {
    if (!trendingInitial || trendingInitial.length === 0) {
      fetchInitialData();
    }
  }, [trendingInitial]);
  ...
  useEffect(() => {
    const fetchSearch = async () => {
      ...
      try {
        setIsSearching(true);
        const res = await axios.get(
          `${BASE_URL}/search/person?api_key=${API_KEY}&include_adult=false&query=${encodeURIComponent(searchQuery)}`
        );
